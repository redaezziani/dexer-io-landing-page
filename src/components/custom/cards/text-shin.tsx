...
interface BadgeTextGradientProps {
    className?: string;
    children?: React.ReactNode;
}
const BadgeTextGradient = ({ className, children }: BadgeTextGradientProps) => {
    return (
        <span className='inline-flex group cursor-pointer items-center justify-center rounded-full border border-slate-400/35 bg-white/15  px-2.5 py-0.5 text-xs font-medium '>
            <span className='bg-gradient-to-t   text-[#8678f9] '>
                {children}
            </span>
            <PartyIcon
                className='size-5 group-hover:ml-3 group-hover:-mt-2 transition-all ease-in-out duration-300 -mt-1 drop-shadow-lg  text-[#8678f9] ml-1'
            />
        </span>
    );
};
...
const PartyIcon = ({className}: {className?: string}) => {
    return (
        ...
        <svg
            className={cn("w-4 h-4 text-[#8678f9] ml-1", className)}
